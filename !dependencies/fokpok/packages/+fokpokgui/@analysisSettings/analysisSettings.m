classdef analysisSettings < handle
    ...
    methods
        function obj = analysisSettings(settings)
            ...
            set(obj.h.chkBox.denoise.removeDCOffset,'Callback',@(hobj,event) obj.guiSetCheckBox('removeDCOffset'))
            ...
            switch type
                case 'denoise'
                    msgbox({'\fontsize{11}{\bfMain}','\color{red}nstddev\color{black} - Determines how many standard deviations of the background noise is removed, cf. ISO11146-3 ch. 3.4.2, n <= 4.',...
                        ...
                        ['\color{red}Remove plane\color{black} - If enabled, background is assumed to be a tilted plane (happens w/ off-axis stray light), cf. ISO11146-3 eq. (54). ',...
                        'DC-Offset subtracts mean background value (determined from edges of ROI). Can be disabled completely for "raw" processing.'],...
        ...
        function guiSetCheckBox(obj,type)
            switch type
                ...
                case 'removeDCOffset'
                    obj.settings.denoise.removeDCoffset = obj.h.chkBox.denoise.removeDCOffset.Value;
                case 'removeplane'
